...
import { Button, Container, Divider, MenuItem, styled, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, useMediaQuery } from "@mui/material";
...
export default function HomePage() {
	...
	const { fetchLocations, submitCount, fetchInventoryCountDetails } = RequestActions;
	const {
		control,
		setFocus,
		handleSubmit,
	...
	let d = { data: { "12279_9": "1", "12279_9_name": "210000010852", "15753_9": "1", "15753_9_name": "2100000251001", "15585_9": "1", "15585_9_name": "210000031737", "15386_9": "1", "15386_9_name": "210000033456", "13409_9": "1", "13409_9_name": "210000030006", "16421_9": "3", "16421_9_name": "2100000372997", "16286_9": "1", "16286_9_name": "210000038964", "16317_9": "1", "16317_9_name": "210000039005", "16358_9": "1", "16358_9_name": "210000039063" }, message: "found", status: 200 };
	...
	const fetchExistingInventory = async () => {
		...
		let existingIvt = await axios.get(`${window.location.href}&type=getCurrentInventoryOfLocation&location=${getValues("location")}`);
		...
		console.log(data)
			...
			Object.keys(data).map((key, i) => {
				...
				if (!key.includes("_name")) {
					...
					append({ itemName: data[`${key}_name`], quantity: Number(data[key]), itemId: key.split("_")[0], status: "Pending" });
				}
			});
		} else
	...
	const onSubmit = async () => {
		try {
			...
			const locationName = allLocations.find((loc) => loc?.id === locationId)?.name;
			const response = await dispatch(submitCount({ items, locationName }));

			if (response?.payload?.data?.status === 200) {
				enqueueSnackbar(response?.payload?.data?.message || "Custom Record created", {
					variant: "success",
				});
				setValue("itemDetails", []);
			} else {
					...
					>
									...
									render={({ field }) => (
										<CustomSelect
											...
											onChange={(e) => {
												...
												setValue("locationName", allLocations.find((loc) => loc?.id === e.target.value)?.name || "");
						...
						<Box>
							<Button
								...
								sx={{ background: "#232323", "&:hover": { background: "black" } }}
							>
								Conduct Full Count
							</Button>
						</Box>
