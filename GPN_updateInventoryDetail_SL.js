...
define([
  ...
  function getParams(context, type) {
    if (type === "getCurrentInventoryOfLocation") {
      ...
      let pageNum = parseInt(context.request.parameters.pageNum);
      let pageSize = parseInt(context.request.parameters.pageSize);
      return { locationId, pageNum, pageSize };
    ...
    try {
      ...
      if (method === "GET") {
        if (type === "role") {
          ...
          log.debug("Page num and pagesize", params.pageNum, params.pageSize);
          ...
          context.response.writeLine(
            JSON.stringify({
              data: currentInventoryCount.data,
              isLast: currentInventoryCount.isLastPage,
              message: "found",
              status: 200,
            })
          );
