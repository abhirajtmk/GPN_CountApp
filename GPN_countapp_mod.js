...
define(["N/record", "N/search", "N/log", "N/format", "N/runtime"], function (record, search, log, format, runtime) {
	...
	function getEnvVariables(requestedInfo, environmentObj = runtime.accountId) {
		...
		try {
			...
			return { error: e.message, "params request": requestedInfo, "string construct": str };
		}
	}
	function HandleSubmitCount(items, locationId, userId, userName, context, locationName, status) {
	...
	function getUsersOfInventoryCount(locationId) {
		try {
			...
			var searchResultCount = customrecordinventory_count_detailsSearchObj.runPaged().count;
			log.debug("customrecordinventory_count_detailsSearchObj result count", searchResultCount);
	...
	function getInventoryCountDetails(userId, date, startIdx = 0, endIdx = 20, locationId) {
		try {
			let filters = [];
			if (userId) {
				filters.push([env.inventoryCountRecordFields.user, "anyof", userId]);
			}
			if (!!date) filters.length ? filters.push("AND", [env.inventoryCountRecordFields.date, "on", formatDate(date)]) : filters.push([env.inventoryCountRecordFields.date, "on", formatDate(date)]);
			if (locationId) filters.length ? filters.push("AND", [env.inventoryCountRecordFields.location, "is", locationId]) : filters.push([env.inventoryCountRecordFields.location, "is", locationId]);
			let customrecordinventory_count_detailsSearchObj = search.create({
				type: env.inventoryCountRecordFields.type,
				filters: filters,
				columns: [
					...
					search.createColumn({ name: env.inventoryCountRecordFields.location, label: "location" }),
				],
			});
			let fetchedDetails = [],
				items = [],
			...
			let searchResult = customrecordinventory_count_detailsSearchObj.run().getRange({
				start: startIdx,
				end: endIdx,
			});
			...
			searchResult.forEach(function (result) {
				...
				details?.items?.map((item) => {
					items.push({ ...item, location: details.location, locationName: details.locationName, userName: details.userName, user: details.user, date: details.date, record: result.getValue("internalid") });
					ids[item.itemId] = location;
				});
			});
			let currentInventoryCounts = [];
			log.debug("ids", Object.keys(ids), Object.values(ids));
			currentInventoryCounts = getCurrentInventoryCountOfItems(Object.keys(ids), Object.values(ids));
	...
	function getCurrentInventoryCountOfItems(itemIdArray, locationIdArray) {
		let filters = [];
		if (itemIdArray.length) filters.push(["internalid", "anyof", ...itemIdArray]);
		if (locationIdArray?.length) {
			if (filters.length) filters.push("AND");
			filters.push(["inventorylocation.internalid", "anyof", ...locationIdArray]);
		}
		if (filters.length) filters.push("AND");
		...
		itemSearchObj.run().each(function (result) {
			...
			itemsQuantityPerLocation[`${itemId}_${location}_name`] = item.split(" ").pop();
			return true;
		});
		return itemsQuantityPerLocation;
	}
	function approveInventoryCount(itemsList, statusInput) {
		try {
			...
			items.forEach((item, i) => {
				groupByRecord[item.record] = groupByRecord[item.record] ? [...groupByRecord[item.record], item] : [item];
			});
			...
			let interoryAjdustments = Object.keys(groupByRecord).map((recordId) => {
				...
				let updatedItems = groupByRecord[recordId].map((item, i) => {
					...
					if (status !== "rejected" && updated_qty_diff != 0) {
						...
						if (i === 0) {
							...
							InventoryAdjustment.setValue({ fieldId: "memo", value: "Inventory Count Reconcilation" });
							InventoryAdjustment.setValue({ fieldId: "trandate", value: new Date(customrecordinventory.getValue(env.inventoryCountRecordFields.date)) });
							InventoryAdjustment.setValue({ fieldId: "subsidiary", value: env.defaultValues.subsidiary });
							InventoryAdjustment.setValue({ fieldId: "account", value: env.defaultValues.adjustmentsAccount });
							InventoryAdjustment.setValue({ fieldId: "adjlocation", value: customrecordinventory.getValue(env.inventoryCountRecordFields.location) });
						}
						if (updated_qty_diff !== 0) {
							log.debug("all fields", [
								...
								{
									fieldId: "trandate",
									value: customrecordinventory.getValue(env.inventoryCountRecordFields.date),
									value2: new Date(customrecordinventory.getValue(env.inventoryCountRecordFields.date)),
									t: new Date(),
								},
								{
									fieldId: "subsidiary",
								...
								{
									fieldId: "adjlocation",
									value: customrecordinventory.getValue(env.inventoryCountRecordFields.location),
								},
								{ sublistId: "inventory", fieldId: "item", value: item.itemId, line: i },
								{ sublistId: "inventory", fieldId: "location", value: item.location, line: i },
								{ sublistId: "inventory", fieldId: "adjustqtyby", value: updated_qty_diff, line: i },
							]);
							InventoryAdjustment.setSublistValue({ sublistId: "inventory", fieldId: "item", value: item.itemId, line: i });
							InventoryAdjustment.setSublistValue({ sublistId: "inventory", fieldId: "location", value: item.location, line: i });
							InventoryAdjustment.setSublistValue({ sublistId: "inventory", fieldId: "adjustqtyby", value: updated_qty_diff, line: i });
							// if (updated_qty_diff < 0) {
							// 	log.debug({
							// 		title: "line",
							// 		details: i,
					...
					updatedItemsUq[`${item.itemId}@${inventoryCount}`] = status === "rejected" ? "Rejected" : "Approved";
					return item;
				});
				let invadjtId = null,
					IAtranId = null;
				if (status !== "rejected" && InventoryAdjustment) {
					...
					invadjtId = InventoryAdjustment.save({ enableSourcing: true, ignoreMandatory: true });
					IAtranId = record.load({ type: "inventoryadjustment", id: invadjtId }).getValue("tranid");
					updatedInventory++;
				...
				let newItms = jsonDetails.items.map((item, i) => {
					let updatedItemStatus = updatedItemsUq[`${item.itemId}@${item.quantity}`];
					log.debug(`${item.itemId}@${item.quantity} ${i}`, { updatedItemStatus });
					return { ...item, status: updatedItemStatus ? updatedItemStatus : item.status };
				});
				...
				return { id: invadjtId || "", location: customrecordinventory.getText(env.inventoryCountRecordFields.location), IAtranId };
				//  else return { id: "", location: "" };
			});
			if (updatedInventory)
				return {
					recordsCreated: interoryAjdustments.map(({ id, location, IAtranId }) => ({
						location: location,
						link: id ? `https://${runtime.accountId.split("_").join("-")}.app.netsuite.com/app/accounting/transactions/invadjst.nl?id=${id}` : "",
						id: IAtranId || "",
			...
			else return { recordsCreated: [], success: true, message: "No Record Created!" };
		} catch (error) {
			log.debug({
				title: "err",
				details: error,
